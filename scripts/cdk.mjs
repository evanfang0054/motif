...
if (code === '--list') {
  const rows = db.prepare('SELECT code, credits, redeemed_by, redeemed_at, created_at FROM cdks ORDER BY created_at DESC').all()
  if (rows.length === 0) {
    console.log('（暂无 CDK）')
  } else {
    for (const r of rows) {
      console.log(`${r.code}  ${r.credits} 张  ${r.redeemed_by ? '已兑换 by ' + r.redeemed_by : '未兑换'}  创建于 ${r.created_at}`)
    }
  }
} else if (code && /^\d+$/.test(String(credits)) && Number(credits) > 0) {
  db.prepare(
    `INSERT INTO cdks (code, credits, created_at) VALUES (?, ?, ?)
     ON CONFLICT(code) DO UPDATE SET credits = excluded.credits, redeemed_by = NULL, redeemed_at = NULL`
