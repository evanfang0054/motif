...
- 充值：套餐（50/100/200/500 张，港元定价 HK$68 起）→ ⚠️ 模拟收银台（见待办）
- CDK：CLI 发码 + 页面兑换（真实）
...
| 优先级 | 事项 | 说明 | 需要的配置 |
| --- | --- | --- | --- |
| 🔴 高 | 真实支付 | 目前为模拟收银台（点确认即到账）。候选渠道：支付宝当面付 / 微信支付 / Stripe / 虎皮椒类个人聚合 | 支付渠道商户资质与密钥 |
| 🔴 高 | 真实发信激活 | Mailer 四渠道已实现，选定渠道（建议 QQ 邮箱 SMTP）并填入授权码即可切换 | 发信邮箱 + 授权码 |
| 🟡 中 | 多管理端能力 | CDK/订单/反馈目前靠 CLI 与数据库查看，缺管理后台页面 | — |
| 🟡 中 | 参考图多图融合策略 | edits 端点已支持多图入参，模板与提示词可针对多参考图进一步调优 | — |
| 🟡 中 | 对象存储 | 图片存本地磁盘；上云需接 S3/OSS 并改 storage 层 | 云存储桶配置 |
| 🟢 低 | 独立 Worker 进程 | 当前队列与应用同进程；高并发场景可拆独立部署（store 层已具备多进程租约语义） | — |
| 🟢 低 | 提示词增强 | `enhance` 字段已贯穿 API，未接 LLM 改写能力 | LLM 对话端点配置 |
| 🟢 低 | 密码找回页面化 | 找回密码已有落地页表单与 API，可增加邮件中的直达链接 | — |
