...
import { resolvePackages, ServiceError, paymentChannel } from '@/server/services'
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (paymentChannel(getRuntime().store) !== 'mock') {
      throw new ServiceError(501, '真实支付渠道接入中，请稍后再试或使用 CDK 兑换额度。')
    }
