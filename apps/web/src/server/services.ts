...
export function register(
  ...
  const signupBonus = Number(values.SIGNUP_BONUS_CREDITS ?? DEFAULT_SIGNUP_BONUS_CREDITS)
  ...
  if (invitedBy) {
    ...
    const reward = inviteRewardFor(inviter.invitedCount, {
      credits: Number(values.INVITE_REWARD_CREDITS ?? DEFAULT_INVITE_REWARD_CREDITS),
      maxInvitees: Number(values.INVITE_REWARD_MAX_INVITEES ?? DEFAULT_INVITE_REWARD_MAX_INVITEES),
    })
