 ...
 *
 * ⚠️ 从 8s 放宽到 30s 的**依据**：上游最大的两个源实测约 1,059,345 B（≈1.01 MiB）与
 * 1,252,781 B（≈1.19 MiB），在 ~100KB/s 的慢链路上需要约 12s —— 8s 下不完，
 * 表现为「部署到服务器后只有这两个源一直抓取失败」。30s 给 2.5 倍余量；
 * 5 个源是并发抓（`refreshSources` 用 `Promise.all`），故管理端「立即刷新」最坏
 * 等约 30s，**不是** 5×30s。
 *
 * 若放宽后**仍然**失败，问题不在带宽：查服务端出网 —— Node 内置 `fetch` 不认
