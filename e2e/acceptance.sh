...
const task = await useOrCreateTaskSpace('motif acceptance')
await ensureRealTab()
const before = await js(String.raw`(() => Number((document.querySelector('.ws-nav').innerText.match(/(?:余额 )?(\d+) 张/) || [])[1]))()`)
...
const after = await js(String.raw`(() => Number((document.querySelector('.ws-nav').innerText.match(/(?:余额 )?(\d+) 张/) || [])[1]))()`)
